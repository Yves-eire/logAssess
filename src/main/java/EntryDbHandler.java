...
public class EntryDbHandler {
    ...
    void insertRecord(Entry entry) throws SQLException {
        try (Connection connection = setConnection(); Statement statement = connection.createStatement();){
            String update = "INSERT INTO ENTRIES VALUES ('" + entry.getId() + "','" + entry.getState()+ "','" +  entry.getType()+ "','" +  entry.getHost()+ "','" + entry.getTimestamp() + "',0)";
            statement.executeUpdate(update);
            connection.commit();
        }
    }

    void alertEntry(String tstamp) throws SQLException {
        try (Connection connection = setConnection(); Statement statement = connection.createStatement();){
            statement.executeUpdate("UPDATE ENTRIES SET alert = 1 WHERE timestamp = "+tstamp);
            connection.commit();
        }
    }
